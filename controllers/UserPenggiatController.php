...
use app\models\search\SearchUserPenggiat;
 ...
 */
class UserPenggiatController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {
        $searchModel = new SearchUserPenggiat();
                ...
                while( ($line = fgetcsv($handle, 1000, ";")) != FALSE) {
                    ...
                    try{
                        if($flag = $model->save(false)) {
                            ...
                            $authAssignment->item_name = 'Penggiat';
                            $authAssignment->save();
    ...
    public function actionView($id)
    {
        ...
        // Data Statistik Penggiat
        
        $categories = ['Penyebarluasan', 'Konsolidasi', 'Pendataaan'];

        $dataPenyebarluasan = Penyebarluasan::find()->where(['created_by' => $id])->count();
        $dataKonsolidasi = Konsolidasi::find()->where(['created_by' => $id])->count();
        $dataPendataan = Pendataan::find()->where(['created_by' => $id])->count();
        ...
        // Map data terakhir
        $lastPenyebarluasan = Penyebarluasan::find()->where(['created_by' => $id])->orderBy(['id'=> SORT_DESC])->one();
        $lastKonsolidasi = Konsolidasi::find()->where(['created_by' => $id])->orderBy(['id'=> SORT_DESC])->one();
        $lastPendataan = Pendataan::find()->where(['created_by' => $id])->orderBy(['id'=> SORT_DESC])->one();

        // var_dump($lastPendataan->latitude);die();
        $lastDatas = [
            'penyebarluasan' => [
                'title' => 'Penyebarluasan',
                'data' => $lastPenyebarluasan,
                'icon' => Url::to('@web/images/penyebarluasan-mark.png')
            ],
            'konsolidasi' => [
                'title' => 'Konsolidasi',
                'data' => $lastKonsolidasi,
                'icon' => Url::to('@web/images/konsolidasi-mark.png')
            ],
            'pendataan' => [
                'title' => 'Pendataan',
                'data' => $lastPendataan,
                'icon' => Url::to('@web/images/pendataan-mark.png')
            ]
        ];
